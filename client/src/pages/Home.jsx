...
const Home = () => {
  ...
  // fetch all products for visible categories
  useEffect(() => {
    if (!categoryData?.length) return;

    const fetchProducts = async () => {
      try {
        const categoryNames = categoryData.map(c => c.name); // or use IDs if backend expects that
        ...
        console.error("Failed to fetch products by categories", err);
      }
    };
      ...
      <div className="container mx-auto px-2 sm:px-4 space-y-10 mt-10 mb-16">
        {filteredCategories?.map((c) => (
          <CategoryWiseProductDisplay
            ...
            id={c._id}
            name={c.name}
            products={productsByCategory[c.name] || []}
            loading={!productsByCategory[c.name]}
          />
        ))}
      </div>
    </section>
